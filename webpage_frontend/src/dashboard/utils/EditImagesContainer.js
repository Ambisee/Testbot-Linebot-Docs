...
export default function EditImagesContainer(props) {
    ...
    const submitFile = () => {
        ...
        fetch('/dropbox-api/upload-image', requestOptions)
            ...
            .then(data => {
                setImages(current => [...current, [data.name, data.link]])
    ...
    const removeFromArray = (name) => {
        const deletionTarget = images.findIndex(obj => obj.name == name);
        setImages(current => current.slice(deletionTarget));
    }
    ...
    return (
        <div className="images-container">
            ...
            <div className="image-gallery">
                {images.length > 0 ?
                    images.map(tup => (<ImageDisplayer key={tup[0]} name={tup[0]} src={tup[1]} onDelete={removeFromArray}/>)) :
