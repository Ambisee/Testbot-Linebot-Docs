...
export default function NavBar(props) {
    ...
    const handleOutsideClick = (e) => {
        if (!(e.target.classList.contains('burger-icon') || e.target.classList.contains('bar'))) {
            document.querySelector('nav').style.marginBottom = '';
            setToggle(false);
        }
    }

    if (toggle) {
        burger += "toggled-burger";
        navList += "toggled-navbar-list";
    }
    ...
    useEffect(() => {
        let value = toggle? document.querySelector('.navbar-list').offsetHeight.toString() + "px" : "";
        document.querySelector('nav').style.marginBottom = value;
    }, [toggle]);
    ...
    return (
        <nav>
            ...
            <a className={burger} onClick={() => {setToggle(!toggle)}}>
