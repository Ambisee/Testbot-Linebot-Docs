...
import { actionCode } from "./_formReducer";

export default function VisibilityToggler({ children, dispatch, state }) {
    return (
        <button
            className="icon-button"
            type="button"
            onMouseDown={() => dispatch({ type: actionCode.TOGGLE_VISIBLE })}
            onMouseLeave={() => dispatch({ type: actionCode.TOGGLE_INVISIBLE })}
            onMouseUp={() => dispatch({ type: actionCode.TOGGLE_INVISIBLE })}
        >
            {state.hidePassword ?
